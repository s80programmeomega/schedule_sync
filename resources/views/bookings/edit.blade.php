                    ...
                    <form method="POST" action="{{ route('bookings.update', $booking) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('event_type_id') is-invalid @enderror" id="event_type_id"
                                ...
                                <option value="{{ $eventType->id }}" {{ (old('event_type_id', $booking->event_type_id)
                                    == $eventType->id) ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status"
                                required>
                                <option value="scheduled" {{ old('status', $booking->status) === 'scheduled' ?
                                    'selected' : '' }}>Scheduled</option>
                                <option value="completed" {{ old('status', $booking->status) === 'completed' ?
                                    'selected' : '' }}>Completed</option>
                                <option value="cancelled" {{ old('status', $booking->status) === 'cancelled' ?
                        ...
                        <div class="mb-4">
                            ...
                            <textarea class="form-control @error('cancellation_reason') is-invalid @enderror"
                                ...
                                rows="2">{{ old('cancellation_reason', $booking->cancellation_reason) }}</textarea>
