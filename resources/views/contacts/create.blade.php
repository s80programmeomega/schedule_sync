...
@section('content')
  <div class="col-lg-10 col-12 py-4 px-4 px-lg-5">
    <div class="mb-4">
      <h1 class="h3 mb-0 fw-bold">Add Contact</h1>
      <p class="text-muted mb-0">Add a new external contact</p>
    </div>
            ...
            <form method="POST" action="{{ route('contacts.store') }}">
                  ...
                  <select class="form-select @error('team_id') is-invalid @enderror" id="team_id" name="team_id">
                    ...
                    @foreach ($teams as $team)
                      <option value="{{ $team->id }}"
                        {{ old('team_id', $selectedTeam) == $team->id ? 'selected' : '' }}>
                        {{ $team->name }}
                      </option>
                    @endforeach
                  </select>
              ...
              <div class="d-flex gap-3">
                <button type="submit" class="btn btn-primary">
                  <i class="bi bi-check me-2"></i>Add Contact
                </button>
                <a href="{{ route('contacts.index', ['team_id' => $selectedTeam]) }}"
